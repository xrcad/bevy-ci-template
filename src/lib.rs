//! Bevy 3D Shapes demo scene.
//! Renders a variety of built-in 3D shape primitives with a UV debug texture.
//! - Press **R** to pause/resume rotation.
//! - Press **Tab** to cycle which row of shapes is in front.
//! - Press **Space** (native only) to toggle wireframe rendering.
            ...
            DefaultPlugins
                ...
                .set(WindowPlugin {
                    primary_window: Some(Window {
                        // On WASM, stretch the canvas to fill the browser viewport.
...
/// Marker component for the rotating shapes.
#[derive(Component)]
struct Shape;
...
fn setup(
    ...
    let ring_extrusions = [
        ...
        meshes.add(Extrusion::new(
            {
                // Approximation: concentric ellipses do not have parallel curves.
    ...
    let mut text = "Press 'R' to pause/resume rotation\nPress 'Tab' to cycle through rows"
        .to_string();
    #[cfg(not(target_arch = "wasm32"))]
...
/// Creates a colorful UV debug texture.
...
/// Which depth row a shape is currently displayed in.
